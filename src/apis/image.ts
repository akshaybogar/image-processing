...
const resizeImage = async (req, res) => {
    ...
    if (!width && !height) {
        return res.send('Height and Width of the image to be displayed not provided!');
    }
    ...
    if (Number.isNaN(width)) {
        return res.send('Provided width value is not integer!');
    }

    if (Number.isNaN(height)) {
        return res.send('Provided height value is not integer!');
    }
